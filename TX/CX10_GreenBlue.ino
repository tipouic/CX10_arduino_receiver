...
void CX10_init() {
	...
	switch(current_protocol) {
		case PROTO_CX10_BLUE:
			for(i=0; i<4; i++) { packet[5+i] = 0xFF; } // clear aircraft ID
	...
	for(i=0; i<4; i++) { CX10_txid[i] = transmitterID[i]; }
	CX10_txid[1] &= 0x2F;
...
void CX10_Write_Packet(uint8_t init) {
	...
	if(ppm[AUX2] > PPM_MID) { packet[12+offset] |= 0x10; } // flip flag
	// rate / mode (use headless channel)
	if(ppm[AUX1] > PPM_MAX_COMMAND) { packet[13+offset] = 0x02; } // mode 3 / headless
	else if(ppm[AUX1] < PPM_MIN_COMMAND) { packet[13+offset] = 0x00; } // mode 1
	else { packet[13+offset] = 0x01; }
	// mode 2
